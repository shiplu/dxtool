...
class WebGet {
    ...
    public function requestContent($url, $getvars=array(), $postvars=array(), $headers=array()) {
        ...
        if (is_array($postvars) && count($postvars) > 0) {
            ...
            curl_setopt(
                    $ch, CURLOPT_POSTFIELDS, http_build_query($postvars)
            );
        } elseif (is_string($postvars) && strlen($postvars) > 0) {
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt(
                    $ch, CURLOPT_POSTFIELDS, $postvars
            );
        }
        if (is_array($headers) && count($headers) > 0) {
            curl_setopt($ch, CURLOPT_HTTPHEADER, $this->buildHeaders($headers, self::HEADER_FORMAT_LINESARRAY)
            );
        }
